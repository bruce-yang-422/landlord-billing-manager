...
### 🎨 使用者體驗
- 🎨 **Apple 玻璃風格設計**：採用最新的 Glassmorphism 設計，半透明毛玻璃效果，現代感十足
- 🌓 **主題切換**：支援亮色（Light）、暗色（Dark）、自動（Auto）三種主題模式，可隨系統設定自動切換
- 📱 **響應式設計**：完美支援手機、平板、電腦，自動適應不同螢幕尺寸
- 🎯 **卡片式布局**：PC 端採用兩欄布局，手機端自動調整為單欄
- 🔘 **滑動開關**：直觀的開關設計，取代傳統 checkbox
- ✨ **內凹輸入框**：輸入框採用內凹設計，視覺層次更清晰
...
2. （選填）選擇主題：點擊右上角的主題選擇器，選擇亮色、暗色或自動模式
3. 填入帳單日期、電錶讀數、租金等資訊
4. （選填）開啟電費新制，填入台電帳單資訊
5. （選填）填入租客備註和房東備註
6. 點擊「計算並存檔」即可完成計算並保存記錄
7. 使用「備份資料」功能匯出 JSON 備份檔
8. 使用「匯入資料」功能還原之前的備份
...
### 樣式檔案
- `css/style.css` - 統一樣式表（Apple 玻璃風格設計，支援亮色/暗色主題）
...
- **資料儲存**：使用 LocalStorage 儲存資料（包含主題偏好設定）
- **響應式設計**：完美支援手機、平板、電腦
- **模組化架構**：CSS 和 JavaScript 分離為多個模組，便於維護與擴展
- **資料格式**：JSON（包含 settings 與 records）
- **向後兼容**：自動處理舊版本資料格式，確保資料不遺失
- **現代化設計**：採用 CSS 變數、Glassmorphism 玻璃風格、平滑過渡動畫
- **主題系統**：使用 CSS 變數實現主題切換，支援系統主題自動偵測
